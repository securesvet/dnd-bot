...
Deno.test("Queue: enqueue operation", () => {
  ...
  assertEquals(queue.getFront(), 10, "Front should return the first element enqueued");
  assertEquals(queue.getRear(), 10, "Rear should return the last element enqueued");
});
...
Deno.test("Queue: enqueue multiple elements", () => {
  ...
  queue.enqueue(30);
  assertEquals(queue.getFront(), 10, "Front should return the first element enqueued");
  assertEquals(queue.getRear(), 30, "Rear should return the last element enqueued");
});
...
Deno.test("Queue: dequeue operation", () => {
  ...
  assertEquals(dequeued?.data, 10, "Dequeue should return the first element enqueued");
  assertEquals(queue.getFront(), 20, "Front should now be the second element enqueued");
});
...
Deno.test("Queue: order is preserved", () => {
  ...
  queue.enqueue("third");
  assertEquals(queue.dequeue()?.data, "first", "The first dequeued element should be the first enqueued");
  assertEquals(queue.dequeue()?.data, "second", "The second dequeued element should be the second enqueued");
  assertEquals(queue.dequeue()?.data, "third", "The third dequeued element should be the third enqueued");
});
